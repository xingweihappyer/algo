...
def train(fileName):
    ...
    #定义观测概率矩阵
    #在ord为所有unicode字符进行编码[example: ord('我')=25105]，大小为 65536，总共4个状态
    #所以B矩阵 4 x 65536
    #就代表每一种状态（词性）得到观测状态（字）
    # 可设为 55303，因为观察值一共有55303
    """
    a = []
    f =open('HMMTrainSet.txt',encoding='utf-8')
    ...
    with open(fileName,encoding='utf-8') as file:
        ...
        # 我们遍历完毕所有的语料，就可以按照公式10.39,40,41来获取A，B，PI     baum-welch
        # 其实这三个公式的本质是 统计出频数/总数
        """
        其实这三个公式的本质是 统计出频数/总数
        """
...
def word_partition(HMM_parameter,article):
    ...
    for line in article:
        ...
        status.append(It)
        # 这是后向的计算该最优路径
        # 所以我们使用 insert ，在列表最前方插入当前算出的最优节点。
